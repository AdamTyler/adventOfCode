...
reading aloud the previous sequence and using that reading as the next sequence. For example, 211 is read as "one two, two ones", which becomes 1221 (1 2, 2 1s).
...
for (var i = 0; i < runs; i++) {
  ...
  for (var j = 0; j < input.length; j++) {
    ...
    newString += count + input[j];
  }
  input = newString;
}

console.log('Result is length:', input.length);
