...
ribbon required for the perfect bow is equal to the cubic feet of volume of the present. Don't
...
function getRibbon(l, w, h) {
  var length = Math.min(2*l + 2*w, 2*l + 2*h, 2*w + 2*h);
  length += l*w*h;
  return length;
}
...
lineReader.on('line', function(line) {
  ...
  lengthRibbon += getRibbon(length, width, height);
});

lineReader.on('close', function() {
  console.log('The elves need ' + sqftPaper + ' sqft of paper');
  console.log('The elves need ' + lengthRibbon + ' feet of ribbon');
});
