...
conveniently number 1 to 500, for sanity) gave you the gift. You open the present and, as luck
would have it, good ol' Aunt Sue got you a My First Crime Scene Analysis Machine! Just what you
...
var data = fs.readFileSync('./inputs/day16.txt').toString().trim().split('\n');
var regex = /Sue (\d+): (.*)/;
var allSues = {};

data.forEach(function(thisSue) {
  var matches = thisSue.match(regex);
  var stuff = matches[2].split(', ');
  allSues[matches[1]] = {};
  for (var i = 0; i < stuff.length; i++) {
    var keyval = stuff[i].split(': ');
    allSues[matches[1]][keyval[0]] = parseInt(keyval[1]);
  }
});

for (var thisSue in allSues) {
  var allKeys = true;
  for (var key in allSues[thisSue]) {
    if (key === 'trees') {
      if (allSues[thisSue][key] <= correctSue[key]) {
        allKeys = false;
      }
    } else if (key === 'cats') {
      if (allSues[thisSue][key] <= correctSue[key]) {
    ...
    } else if (allSues[thisSue][key] !== correctSue[key]) {
      allKeys = false;
    }
  }
  if (allKeys) {
    console.log('correct Sue is', thisSue);
    break;
  }
}
